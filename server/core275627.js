...
var i=write_sceen_time-r,s=first_sceen__time-r,d=page_endtime-r,m=window.logs.jsapi_ready_time?window.logs.jsapi_ready_time-r:void 0,w=window.logs.a8key_ready_time?window.logs.a8key_ready_time-r:void 0,g=c&&c.connectEnd-c.connectStart,p=c&&c.secureConnectionStart&&c.connectEnd-c.secureConnectionStart;
...
o.setFlags(1636,1,1);
for(var _ in u)!u[_]||u[_]<0||o.setPoint(_,u[_]);
o.report();
}
...
void t.send();
t.setAvg(27822,1,d).setAvg(27822,3,i).setAvg(27822,5,s),window.is_page_cached&&t.setAvg(27822,19,d),
e()?(t.setAvg(27822,9,d),window.is_page_cached&&t.setAvg(27822,23,d)):"wifi"==networkType?(t.setAvg(27822,7,d),
window.is_page_cached&&t.setAvg(27822,21,d)):"2g/3g"==networkType?(t.setAvg(27822,11,d),
window.is_page_cached&&t.setAvg(27822,25,d)):(t.setAvg(27822,13,d),window.is_page_cached&&t.setAvg(27822,28,d)),
t.send();
}
}(),function(){
...
if(a+n>=r){
for(var d,m,w=t.getElementsByTagName("img"),g={},p=[],c=0,u=0,_=0,f=0,l=w.length;l>f;++f){
var v=w[f];
d=v.getAttribute("data-src")||v.getAttribute("src"),m=v.getAttribute("src"),d&&(d.isCDN()?u++:_++,
c++,g[m]={});
}
if(p.push("1="+1e3*c),p.push("2="+1e3*u),p.push("3="+1e3*_),s.getEntries){
var h=s.getEntries(),y=window.logs.img.download,j=[0,0,0],A=[0,0,0];
c=u=0;
for(var f=0,b=h.length;b>f;++f){
var k=h[f],T=k.name;
...
for(var M=y[f],E=0,z=0,x=0,S=0,C=0,l=M.length;l>C;++C){
var d=M[C];
if(g[d]&&g[d].startTime&&g[d].responseEnd){
var N=g[d].startTime,I=g[d].responseEnd;
E=Math.max(E,I),z=z?Math.min(z,N):N,d.isCDN()&&(x=Math.max(E,I),S=z?Math.min(z,N):N);
}
}
j[1]+=Math.round(E-z),A[1]+=Math.round(x-S);
}
for(var P=4,B=7,f=0;3>f;f++)j[f]=Math.round(j[f]),A[f]=Math.round(A[f]),j[f]>0&&(p.push(P+f+"="+j[f]),
"wifi"==networkType?p.push(P+f+6+"="+j[f]):"2g/3g"==networkType&&p.push(P+f+12+"="+j[f])),
A[f]>0&&(p.push(B+f+"="+A[f]),"wifi"==networkType?p.push(B+f+6+"="+A[f]):"2g/3g"==networkType&&p.push(B+f+12+"="+A[f]));
}
...
t.title=s(decodeURIComponent(e.getAttribute("name")||"")),d.renderPlayer("voice_tpl",t,e),
...
i.music_name=s(e.getAttribute("music_name")||""),a.renderPlayer("qqmusic_tpl",i,e),
